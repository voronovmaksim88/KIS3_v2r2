  ...
  <div class="task-node border rounded-md p-2" :class="{'bg-gray-700': isExpanded, 'hover:bg-gray-700': !isExpanded}">
      ...
      <div class="flex items-center">
        ...
        <div>
          ...
          <div class="flex text-xs text-gray-400 space-x-2">
            <span>{{ statusMap[task.status_id] || 'Статус не указан' }}</span>
            <span v-if="task.executor_id">| Исполнитель: {{ task.executor_id }}</span>
          </div>
        </div>
      </div>
      <Badge v-if="task.payment_status_id" :value="paymentStatusMap[task.payment_status_id]"
             :severity="getPaymentStatusSeverity(task.payment_status_id)" />
    </div>
    ...
    <div v-if="isExpanded" class="pl-4 mt-2">
      ...
      <div class="text-xs text-gray-400 grid grid-cols-2 gap-2 mb-2">
        <div v-if="task.planned_duration">План: {{ formatDuration(task.planned_duration) }}</div>
        <div v-if="task.actual_duration">Факт: {{ formatDuration(task.actual_duration) }}</div>
        <div v-if="task.creation_moment">Создана: {{ formatDate(task.creation_moment) }}</div>
        <div v-if="task.start_moment">Начата: {{ formatDate(task.start_moment) }}</div>
        <div v-if="task.deadline_moment">Срок: {{ formatDate(task.deadline_moment) }}</div>
        <div v-if="task.end_moment">Завершена: {{ formatDate(task.end_moment) }}</div>
        <div v-if="task.price">Стоимость: {{ task.price }} ₽</div>
      </div>
